...
function TopicForm({ setResult, setLoading, loading, setError }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // ✅ Real-time Redux Credit update (Top Header credits auto-reduce honge)
      ...
      const errorMsg = error?.response?.data?.message || "An error occurred while generating notes. Please try again.";
      setError(errorMsg);
      setLoading(false);
    }
  };
        ...
        className={`
          ...
          ${
            loading
              ...
              : "bg-gradient-to-br from-white to-gray-200 text-black shadow-[0_15px_35px_rgba(0,0,0,0.4)] hover:opacity-95"
          }
        `}>
    ...
    <div className='flex items-center gap-4 cursor-pointer select-none' onClick={onChange}>
      ...
      >
        <motion.div
          ...
          className='absolute top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-white shadow-[0_5px_15px_rgba(0,0,0,0.5)]'
          style={{
            left: checked ? "calc(100% - 1.35rem)" : "0.15rem",
          }}
        />
      </motion.div>
